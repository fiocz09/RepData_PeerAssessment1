...
### 1.1.Load the data
```{r results = "hide"}
download.file("https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2Factivity.zip", dest = "activity.zip")
unzip("activity.zip")
...
```
The **mean** total number of steps taken per day is **`r mean.raw.stps`**, and the **median** total number of steps taken per day is **`r medi.raw.stps`**.
...
stp.by.intv <- sapply(raw.by.intv, function(x) 
                      mean(x$steps, na.rm = T))
plot(names(stp.by.intv), stp.by.intv, type = "l", 
...
```
The **`r nam.stps`** 5-minute interval, on average across all the days in the dataset, contains the **maximum number of steps**, which is **`r max.stps`** steps.
...
```{r}
library(plyr)
act <- ddply(raw, .(interval), transform, 
...
hist(act.stps, breaks = 10, 
     main = "total number of steps taken each day", 
     xlab = "steps")
mean.act.stps <- mean(act.stps, na.rm = T)
medi.act.stps <- median(act.stps, na.rm = T)
```
After filling in the missing values, the **mean** total number of steps taken per day is **`r mean.act.stps`**, and the **median** total number of steps taken per day is **`r medi.act.stps`**.
...
act.by.wkdy.by.intv <- lapply(act.by.wkdy, 
                              function(x) split(x, x$interval))
fina <- data.frame(interval = as.integer(rep(names(table(act$interval)), 
                                        times = 2)), 
                   steps = c(sapply(act.by.wkdy.by.intv[[1]], 
                                   function(x) mean(x$steps)), 
                            sapply(act.by.wkdy.by.intv[[2]], 
                                  function(x) mean(x$steps))), 
                   weekdays = rep(c("weekday", "weekend"), 
                                  each = 288))
library(lattice)
xyplot(steps ~ interval | weekdays, data = fina, type = "l",
       layout = c(1, 2), 
